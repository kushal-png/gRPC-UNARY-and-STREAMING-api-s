...
type GreetServiceClient interface {
	...
	SayHelloBidirectionStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloBidirectionStreamingClient, error)
}
...
func (c *greetServiceClient) SayHelloBidirectionStreaming(ctx context.Context, opts ...grpc.CallOption) (GreetService_SayHelloBidirectionStreamingClient, error) {
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[2], "/greet_services.GreetService/SayHelloBidirectionStreaming", opts...)
	if err != nil {
		return nil, err
	}
	x := &greetServiceSayHelloBidirectionStreamingClient{stream}
	return x, nil
}

type GreetService_SayHelloBidirectionStreamingClient interface {
	Send(*HelloRequest) error
	Recv() (*HelloResponse, error)
	grpc.ClientStream
}

type greetServiceSayHelloBidirectionStreamingClient struct {
	grpc.ClientStream
}

func (x *greetServiceSayHelloBidirectionStreamingClient) Send(m *HelloRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *greetServiceSayHelloBidirectionStreamingClient) Recv() (*HelloResponse, error) {
	m := new(HelloResponse)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type GreetServiceServer interface {
	...
	SayHelloBidirectionStreaming(GreetService_SayHelloBidirectionStreamingServer) error
...
func (UnimplementedGreetServiceServer) SayHelloBidirectionStreaming(GreetService_SayHelloBidirectionStreamingServer) error {
...
func _GreetService_SayHelloBidirectionStreaming_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(GreetServiceServer).SayHelloBidirectionStreaming(&greetServiceSayHelloBidirectionStreamingServer{stream})
}

type GreetService_SayHelloBidirectionStreamingServer interface {
	Send(*HelloResponse) error
	Recv() (*HelloRequest, error)
	grpc.ServerStream
}

type greetServiceSayHelloBidirectionStreamingServer struct {
	grpc.ServerStream
}

func (x *greetServiceSayHelloBidirectionStreamingServer) Send(m *HelloResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *greetServiceSayHelloBidirectionStreamingServer) Recv() (*HelloRequest, error) {
	m := new(HelloRequest)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
